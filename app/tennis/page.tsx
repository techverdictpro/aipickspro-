...
const LEAGUE_META: Record<string, { label: string; flag: string }> = {
  'roland-garros':    { label: 'Roland Garros', flag: '🇫🇷' },
  'atp':              { label: 'ATP Tour', flag: '🎾' },
  'wta':              { label: 'WTA Tour', flag: '🎾' },
  'wimbledon':        { label: 'Wimbledon', flag: '🇬🇧' },
  'us-open':          { label: 'US Open', flag: '🇺🇸' },
  'australian-open':  { label: 'Australian Open', flag: '🇦🇺' },
}
...
function getArticles(): Record<string, Article[]> {
  const baseDir = path.join(process.cwd(), 'content', 'predictions', 'tennis')
  ...
  for (const entry of entries) {
    ...
    if (entry.endsWith('.mdx')) {
      ...
      if (data.date >= weekAgo) {
        if (!result['atp']) result['atp'] = []
        result['atp'].push({ slug: entry.replace('.mdx', ''), ...data, content } as Article)
      }
    ...
    <>
      ...
      <div className="page-header">
        <div className="page-header-inner">
          ...
          <p className="page-subtitle">Expert AI picks for ATP, WTA and Grand Slams — {total} tips this week</p>
        </div>
      </div>
      ...
      <div className="main">
        {leagues.length === 0 ? (
          ...
          leagues.map(leagueSlug => {
            ...
            const label = meta?.label || leagueSlug.toUpperCase().replace(/-/g, ' ')
