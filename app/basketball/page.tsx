...
function getAllArticles(): Record<string, Article[]> {
  const baseDir = path.join(process.cwd(), 'content', 'predictions', 'basketball')
  ...
  for (const entry of entries) {
    ...
    if (fs.statSync(entryPath).isDirectory()) {
      ...
      for (const file of files) {
        ...
        if (data.date >= weekAgo) articles.push({ slug: file.replace('.mdx', ''), title: data.title || '', league: data.league || entry, date: data.date || '', prediction: data.prediction || '', odds: data.odds || '', confidence: data.confidence || 3, content })
      }
      if (articles.length > 0) result[entry] = articles.sort((a, b) => new Date(b.date).getTime() - new Date(a.date).getTime())
    } else if (entry.endsWith('.mdx')) {
      const raw = fs.readFileSync(entryPath, 'utf-8')
      const { data, content } = matter(raw)
      if (data.date >= weekAgo) {
        if (!result['nba']) result['nba'] = []
        result['nba'].push({ slug: entry.replace('.mdx', ''), title: data.title || '', league: data.league || 'NBA', date: data.date || '', prediction: data.prediction || '', odds: data.odds || '', confidence: data.confidence || 3, content })
      }
    }
  }
  return result
    ...
    <>
      ...
      <div className="page-header">
        <div className="page-header-inner">
          <div className="breadcrumb"><a href="/">Home</a> &rsaquo; <span style={{color:'#e8f042'}}>&#127936; Basketball Predictions</span></div>
          <h1 className="page-title">Basketball <em>Predictions</em></h1>
          <p className="page-subtitle">AI-powered NBA picks with expert stats analysis and best odds — {total} tips this week</p>
        </div>
      </div>
      ...
      <div className="main">
        {leagues.length === 0 ? (
          ...
          leagues.map(leagueSlug => {
            ...
            const label = leagueSlug.toUpperCase().replace(/-/g, ' ')
            return (
